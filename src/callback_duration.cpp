...
callback_duration_graph_is_configured(bt_self_component_sink *self_component_sink)
{
    ...
    bt_message_iterator_create_from_sink_component(self_component_sink,
        in_port, &callback_duration->message_iterator);
...
void callback_duration_calculator(struct CallbackDuration *callback_duration, const bt_message *message)
{
    ...
    if (strcmp(event_name, "ros2:rclcpp_callback_register") == 0) {
        ...
        callback_duration->callback_durations[callback_address_value].emplace_back(last_called, ns_from_origin - last_called);
        ...
        bt_self_component_sink *self_component_sink)
{
    ...
    bt_message_iterator_next_status next_status =
        bt_message_iterator_next(callback_duration->message_iterator, &messages,
            &message_count);
...
/* Define the `callback_duration` plugin */
BT_PLUGIN(callback_duration);

/* Define the `output` sink component class */
BT_PLUGIN_SINK_COMPONENT_CLASS(output, callback_durations_consume);
...
BT_PLUGIN_SINK_COMPONENT_CLASS_FINALIZE_METHOD(output, callback_duration_finalize);
